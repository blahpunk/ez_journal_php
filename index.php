...
function load_dotenv_file(string $path): void
{
    ...
    foreach ($lines as $line) {
        ...
        if ($rawValue === '') {
            ...
            $value = $commentPos === false ? $rawValue : rtrim(substr($rawValue, 0, $commentPos));
        }
...
$pinUserIsEditor = filter_var(
    (string) (getenv('JOURNAL_FALLBACK_ADMIN_IS_EDITOR') ?: '1'),
...
$secureAuthSecrets = [];
if ($secureAuthSecret !== '') {
    $secureAuthSecrets[] = $secureAuthSecret;
}
if ($secureAuthPreviousSecretsRaw !== '') {
    foreach (explode(',', $secureAuthPreviousSecretsRaw) as $part) {
        $candidate = trim($part);
        if ($candidate !== '') {
            $secureAuthSecrets[] = $candidate;
        }
    }
}
...
): void
{
    ...
    if ($jId === null) {
        $insertPinUser = $db->prepare(
            'INSERT INTO user (label, pin_hash, is_editor, email, name, auth_type)
             VALUES (:label, :pin_hash, :is_editor, NULL, :name, :auth_type)'
        );
        $insertPinUser->execute([
            ':label' => $pinUserLabel,
            ':pin_hash' => hash_pin($pinUserPin),
            ':is_editor' => $pinUserIsEditor ? 1 : 0,
            ':name' => $pinUserLabel,
            ':auth_type' => 'pin',
        ]);
    ...
    $needsPinUserUpdate =
        ...
        || (int) ($pinUserState['is_editor'] ?? 0) !== ($pinUserIsEditor ? 1 : 0)
 ...
 */
function oauth_identity_from_cookie(array $secrets): ?array
{
...
function layout(string $content, ?array $currentUser, array $recentEntries, string $title = 'BlahPunk Blog'): void
{
    ...
    $stylePath = __DIR__ . '/static/styles.css';
    if (is_file($stylePath)) {
        $styleMtime = @filemtime($stylePath);
        if ($styleMtime !== false) {
            $styleHref .= '?cb=' . rawurlencode((string) $styleMtime);
        }
    }
    ?>
<!DOCTYPE html>
